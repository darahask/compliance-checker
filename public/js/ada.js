let loadADA = (data, url) => {
    ...
    `
        ...
        if(data.tab_Violations.tabIndexViolations.length){
            HTML += `<strong style="color:red">${data.tab_Violations.tabIndexViolations.length}</strong>`
        }else{
            HTML += `<strong style="color:green">${data.tab_Violations.tabIndexViolations.length}</strong>`
        }
        HTML += `</br><span>InterativeElements Violations: `
        if(data.tab_Violations.intViolations.length){
            HTML += `<strong style="color:red">${data.tab_Violations.intViolations.length}</strong></span>`
        }else{
            HTML += `<strong style="color:green">${data.tab_Violations.intViolations.length}</strong></span></p>`
        }
        HTML +=  `</button>
                </h2>
